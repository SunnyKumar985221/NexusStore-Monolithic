...
import { fetchUser } from "../../redux/reducers/usertk";
...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      let response = await axios.post(`http://localhost:8000/user/login-user`, { email, password }, { withCredentials: true });
      if (response) {
    ...
    <form onSubmit={handleSubmit}>
      <div className="login">
        ...
        <div className="loginright">
          <input className="inputlogin" ref={inputref} name='email' onInput={e => setEmail(e.target.value)} placeholder="Email address or phone number" type="email" />
